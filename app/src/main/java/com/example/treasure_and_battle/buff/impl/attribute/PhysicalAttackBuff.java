...
/**
 * 物理攻击力属性Buff：单一职责，只负责物理攻击力属性的加成
 */
public class PhysicalAttackBuff extends BaseBuff {
    private final AttributeModifierType modifierType;

    public PhysicalAttackBuff(String buffId, String buffName, String descriptionFormat,
    ...
    public void applyAttributeBonus(AttributeSet attributeSet) {
        ...
        if (modifierType == AttributeModifierType.FLAT) {
            attributeSet.physicalAtk += (int) finalValue;
        }
        // 2. 百分比物理攻击力加成
        else if (modifierType == AttributeModifierType.PERCENTAGE) {
            attributeSet.percentPhysicalAtk += finalValue;
        }
    }
